...
The output script differs from the style of output generated by the augtool `print` command, in that
the `print` command output always uses an absolute position (number) for repeated nodes or sequential nodes
...
c) If there is an existing entry for `127.0.0.1` in `/etc/hosts` which is different to the above, it will be modified
   using the values from `augsuggest` output, regardless of whether or not `127.0.0.1` is the first entry in the file
...
The nature of the `augsuggest` paths are that they are position-independent, so that it doesn't require `127.0.0.1` to be 
...
eg. multiplying a number by zero or one, or taking the zero-th power of a number.
...
However, it will not be able to _create_ the paths which are of the form `.../1` or `.../2` etc
...
This statement will _not_ create a new entry for `127.0.0.1` if none exists.
...
ie if the entry for `192.0.2.3` does not already exist, it will be created as the _last_ entry in `/etc/hosts`
...
`augsuggest` tries hard to find a path-expression which is unique to a position, and independent of the numerical position
